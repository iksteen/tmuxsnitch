...
- `viewer/viewer.ts` — **the** renderer (TypeScript); nothing is painted server-side. Applies full/diff/banner messages to an in-memory cell grid and re-renders dirty rows: run coalescing, absolute `left:{col}ch` positioning, palette/dim/reverse math. Box-drawing + block glyphs (U+2500–259F) are drawn as **procedural SVG geometry** (`glyphGeometry`, kitty-style: filled rects/arcs/polys in a `viewBox="0 0 1 1"` + `preserveAspectRatio="none"` unit box, per-axis thickness from cell metrics measured at boot) — font-independent, exact tiling. Other symbol/fill glyphs still stretch a font glyph via `textLength`; solid-line/shade runs merge into one span. Geometry beats `symbol_map` except in the PUA. Pages ship an empty `#screen`; the first SSE event after the version hello is always a full frame, so the initial paint lands one round-trip in. `build.rs` compiles it (local `tsc`, else the committed `viewer/dist/viewer.js`), bakes it in via `include_str!`, and both servers serve it at `/viewer.js`.
